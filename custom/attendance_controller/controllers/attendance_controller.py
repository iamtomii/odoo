...
class AttendanceController(http.Controller):
    ...
    def get_time_checkout(self,rfid,**kw):
        ...
        if employee_rfid:
            ...
            return "false"
    @http.route('/attendance_controller/test_api', type='json', auth='public', methods=['POST'],
                csrf=False)
    def update_forget_checkout_test(self):
        data_request=json.loads(request.httprequest.data)
        print("1")
        id_attendance=http.request.env['attendance.video'].search([["attendance_id","=",data_request['id']]])
        print("2")
